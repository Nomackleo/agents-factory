...
def get_resolve_instance():
    ...
    if RESOLVE_SCRIPT_MODULE_PATH not in sys.path and os.path.exists(RESOLVE_SCRIPT_MODULE_PATH):
...
def davinci_get_status() -> Dict[str, Any]:
    ...
    if resolve:
        ...
        proj_name = current_project.GetName() if current_project else "Sin proyecto abierto"
        return {
            "status": "connected",
...
def davinci_create_timeline(project_name: str, timeline_name: str, media_paths: List[str]) -> Dict[str, Any]:
    ...
    if not project:
        return {"status": "error", "message": f"No se pudo crear/cargar el proyecto '{project_name}'"}
    ...
    timeline = media_pool.CreateTimelineFromClips(timeline_name, imported_clips) if imported_clips else media_pool.CreateEmptyTimeline(timeline_name)
...
def davinci_render_timeline(project_name: str, timeline_name: str, target_dir: str, render_preset: str = "H.264 Master") -> Dict[str, Any]:
    ...
    if not project:
        return {"status": "error", "message": f"Proyecto '{project_name}' no encontrado"}

    project.SetCurrentRenderFormatAndCodec("mp4", "H264")
    project.SetRenderSettings({"TargetDir": target_dir, "CustomName": f"{timeline_name}_master"})
